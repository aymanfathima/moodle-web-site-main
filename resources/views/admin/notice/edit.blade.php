...
@section('content')
    ...
    <div class="form-wrap">
        <form action="{{ route('admin_notice_update', ['id' => $notice->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row gy-2 gx-3">
                ...
                <div class="col-12 col-lg-6 col-xl-4">
                    <div>
                        ...
                        <input type="time" class="form-control" id="start_time" name="start_time"
                            value="{{ old('start_time', $notice->start_time) }}">
                    </div>
                    @error('start_time')
                        <span class="error">{{ $message }}</span>
                    @enderror
                ...
                <div class="col-12 col-lg-6 col-xl-4">
                    <div>
                        ...
                        <input type="time" class="form-control" id="end_time" name="end_time"
                            value="{{ old('end_time', $notice->end_time) }}">
                    </div>
                    @error('end_time')
                        <span class="error">{{ $message }}</span>
                    @enderror
                ...
                <div class="col-12 col-lg-6 col-xl-4">
                    <div>
                        ...
                        <select class="form-select" name="state">
                            <option value="1" {{ old('state', $notice->state) == 1 ? 'selected' : '' }}>Active
                            </option>
                            <option value="0" {{ old('state', $notice->state) == 0 ? 'selected' : '' }}>Inactive
                            </option>
                        </select>
                    </div>
                    @error('state')
                ...
                <div class="col-12 d-flex justify-content-end">
                    <button class="btn btn-primary" type="submit"><i class="bi bi-floppy-fill"></i> Update
                        Notice</button>
                </div>
