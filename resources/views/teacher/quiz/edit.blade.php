...
@section('content')
    ...
    <div class="form-wrap">
        <form action="{{ route('teacher_quiz_update', ['id' => $quiz->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row gy-2 gx-3">
                <div class="col-12 col-lg-6 col-xl-4">
                    <div>
                        <label for="id" class="form-label">Quiz ID</label>
                        <input type="text" class="form-control" id="id" name="id"
                            value="{{ old('id', $quiz->id) }}" readonly disabled>
                    </div>
                </div>
                <div class="col-12 col-lg-6 col-xl-4">
                    <div>
                ...
                <div class="col-12 col-lg-6 col-xl-4">
                    <div>
                        ...
                        <input type="number" class="form-control" id="attempts" name="attempts"
                            value="{{ old('attempts', $quiz->attempts) }}">
                ...
                <div class="col-12 col-lg-6 col-xl-4">
                    <div>
                        ...
                        <select class="form-select" id="state" name="state">
                            <option value="1" {{ old('state', $quiz->state) == 1 ? 'selected' : '' }}>Active
                            </option>
                            <option value="0" {{ old('state', $quiz->state) == 0 ? 'selected' : '' }}>Inactive
                            </option>
                        </select>
                    </div>
                    @error('state')
