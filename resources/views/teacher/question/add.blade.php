...
@section('content')
    ...
    <div class="btn-wrap mb-3">
        <a href="{{ route('teacher_question_index', ['id' => $id]) }}"><i class="bi bi-caret-left-fill"></i> Go Back</a>
    </div>

    <div class="form-wrap">
        <form action="{{ route('teacher_question_store') }}" method="POST">
            @csrf
            <div class="row gy-2 gx-3">
                <div class="col-12 col-lg-6 col-xl-4">
                    <div>
                        ...
                        <input type="hidden" class="form-control" value="{{ $id }}" name="quiz_id">
                    </div>
                    @error('quiz_id')
                ...
                <div class="col-12 col-lg-6 col-xl-4">
                    <div>
                        ...
                        <select name="answer" id="answer" class="form-select">
                            <option id="selectOption1" value="1" @selected(old('answer') == 1)>Option 1</option>
                            <option id="selectOption2" value="2" @selected(old('answer') == 2)>Option 2</option>
                            <option id="selectOption3" value="3" @selected(old('answer') == 3)>Option 3</option>
                            <option id="selectOption4" value="4" @selected(old('answer') == 4)>Option 4</option>
                        </select>
                    </div>
                    @error('answer')
                        <span class="error">{{ $message }}</span>
